...
class Event
{
     ...
     * @Assert\MaxLength(limit=200, message="error.event.name.too_long")
     */
    private $name;
     ...
     *
     * @ORM\OneToOne(targetEntity="Droppy\EventBundle\Entity\EventDateTime", cascade={"persist","remove"})
     * @ORM\JoinColumn(name="end_date_time_id", referencedColumnName="id", nullable=false)
     * @Assert\Valid()
     */
    private $endDateTime;
     ...
     *
     * @ORM\OneToMany(targetEntity="Droppy\UserBundle\Entity\UserDropsEventRelation", mappedBy="event", cascade={"persist","remove"})
     */
    ...
    {
        if($this->startDateTime->compare($this->endDateTime) === 1)
        {
            $propertyPath = $context->getPropertyPath() . '.endDateTime';
            $context->setPropertyPath($propertyPath);
            $context->addViolation('error.date.not_interval', array(), null);
        }
    }
     ...
     * @ORM\PrePersist()
     */
    public function changeLastUpdate()
    {
        $this->lastUpdate = new \DateTime();
    }
